...
public class CustomerMenuController {
  ...
  private List<MenuElement> generateChildren(
      List<Customer> allCustomers, List<MenuElement> currentMenu) {
    return allCustomers.stream()
        .map(
            customer -> {
              MenuElement customerElement =
                  new MenuElement(customer.getId(), customer.getName(), null);
